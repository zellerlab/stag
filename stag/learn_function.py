...
def main():
    ...
    ap.add_argument(
        '-e', action="store", default="l1", dest='penalty_logistic',
        help='penalty for the logistic regression', choices=['l1', 'l2', 'none']
    )
    ap.add_argument(
        '-E', action="store", default="liblinear", dest='solver_logistic',
        help='solver for the logistic regression', choices=['newton-cg', 'lbfgs', 'liblinear', 'sag', 'saga']
    )
    ...
    all_calc_functions = learn_function(
        args.level, alignment, full_taxonomy, penalty_v, solver_v,
        gene_level=args.level == n_levels, procs=procs
    )
