...
def create_db(aligned_seq_file, tax_file, verbose_, output, use_cmalign, hmm_file_path, save_cross_val_data, protein_fasta_input, penalty_v, solver_v, NN_start_level,intermediate_dist_for_NN,min_training_data_lmnn,skip_training_1, base_save_features, procs=None):
    # set verbose
    global verbose
    verbose = verbose_
    ...
    all_LMNN, thresholds_NN, centroid_seq, species_to_tax, all_sel_positions = train_NN_classifiers(alignment, tax_file, NN_start_level, logging, verbose,intermediate_dist_for_NN,min_training_data_lmnn, base_save_features, procs=procs if procs else 1)
